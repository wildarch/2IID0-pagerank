function answer = framework( A, usingdegreeofnodes )
%calculates the value-based-error for every itteration based on random
%adding of nodes or based on the random adding of nodes with respect to the
%degree of those nodes.
...
if usingdegreeofnodes == 0
    edge_indices = randperm(length(A));
elseif usingdegreeofnodes == 1
    uniq = unique(A(:,2));
